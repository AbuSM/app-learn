...
import shortTheTitle from "../../../api/shortTheTitle.js";
...
export const renderTasks = (tasks, isInitial = false) => {
  ...
  tasks.forEach((element, listIndex) => {
    ...
    console.log(shortTheTitle(element.title));

    htmlDataLists += /*html*/ `
      <ul
        draggable="true"
        data-list-index="${listIndex}"
        class="list max-h-full [&.draggable]:opacity-50 gap-2 transition-all [&.droppable]:border-[var(--primary)] bg-white flex min-w-[var(--card-width)] max-w-[var(--card-width)] flex-col shadow border-2 border-[var(--border-gray)] rounded-xl p-3"
      ...
      >
        ...
        >
          <h3 onclick="window.onListHeadingClick(event)" class="w-full ml-2 text-xl font-bold">${shortTheTitle(element.title)}</h3>
